show databases;
create database personal_financial_health;
use personal_financial_health;
show tables;
create table users(id int primary key auto_increment, username varchar(20) unique not null, role enum('admin','user'), password varchar(25) not null);
create table income(income_id int primary key auto_increment, id int, source varchar(100), amount decimal(10,2) not null, date date, foreign key(id) references users(id));
create table expenses(expense_id int primary key auto_increment, id int, category varchar(40), amount decimal(10,2) not null, date date, foreign key(id) references users(id));
create table savings(saving_id int primary key auto_increment, id int, account_type varchar(50) not null, amount decimal(10,2) not null, date date, foreign key(id) references users(id));
create table investments(invetsment_id int primary key auto_increment, id int, type varchar(50), amount decimal(10,2) not null, return_rate decimal(5,2), date date, foreign key(id) references users(id));
create table debts(debt_id int primary key auto_increment, id int, type varchar(50), amount decimal(10,2) not null, interest_rate decimal(5,2), due_date date, foreign key(id) references users(id));
create table financial_health(health_id int primary key auto_increment, id int, score decimal(5,2), date date, foreign key(id) references users(id));
...
INSERT INTO Savings (id, account_type, amount, date) VALUES
-- John Doe
...
select * from savings;
INSERT INTO Investments (id, type, amount, return_rate, date) VALUES
-- John Doe
...
INSERT INTO Debts (id, type, amount, interest_rate, due_date) VALUES
-- John Doe
...
select * from debts;
select users.username, sum(income.amount) - sum(expenses.amount) as net_income from users left join income on users.id = income.id left join expenses on users.id = expenses.id where income.date between '2023-10-01' and '2023-10-31' and username = 'john_doe' group by users.id;
select users.username, debts.type, debts.amount, debts.interest_rate from debts inner join users on debts.id = users.id where debts.interest_rate > 10 order by debts.interest_rate desc;
select users.id, (sum(income.amount) - sum(expenses.amount) - sum(debts.amount))/sum(income.amount)*100 as score, now() from users left join income on users.id = income.id left join expenses on users.id = expenses.id left join debts on users.id = debts.id group by users.id;
select users.username, expenses.category, avg(expenses.amount) as avg_spending from expenses inner join users on expenses.id = users.id group by users.id, expenses.category having avg_spending > (select avg(amount) from expenses where category = 'entertainment');
select users.id, savings.account_type, (savings.amount/(select sum(amount) from savings where id = savings.id))*100 as growth_rate from savings inner join users on savings.id = users.id; 
create user 'admin'@'localhost' identified by 'admin123';
show grants for 'admin'@'localhost';
grant all on personal_financial_health.* to 'admin'@'localhost' with grant option;
create user 'user'@'localhost' identified by 'user123';
grant select on personal_financial_health.* to 'user'@'localhost';
...
select * from income;
savepoint s1;
insert into income(id,source,amount,date) values(1,'side hustle',9000.00,'2023-10-11');
savepoint s2;
update savings set amount = amount + 9000.00 where id = 1 and account_type = 'retirement fund';
commit;
start transaction;
select * from debts;
select * from savings;
savepoint s3;
update savings set amount = amount - 1000.00 where id = 1 and account_type = 'emergency fund';
savepoint s4;
update debts set amount = amount - 1000.00 where id = 1;
commit;
start transaction;
select * from savings;
savepoint s5;
update savings set amount = amount - 500.00 where id = 1 and account_type = 'emergency fund';
savepoint s6;
update savings set amount = amount + 500.00 where id = 1 and account_type = 'retirement';
commit;
start transaction;
select * from investments;
select * from savings;
savepoint s7;
update savings set amount = amount - 2000 where id = 1 and account_type = 'emergency fund';
savepoint s8;
insert into investments(id,type,amount,return_rate,date) values(1,'mutual funds', 2000.00, 6.00, '2023-10-03');
commit;